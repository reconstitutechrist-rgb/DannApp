...
export const ExamplePrompts: React.FC<ExamplePromptsProps> = ({
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-medium text-neutral-300">Try these ideas:</h3>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {displayPrompts.map((prompt) => (
            <button
              ...
              className="glass-subtle rounded-xl p-4 text-left hover:bg-white/5 transition-all border border-transparent hover:border-primary-500/30 group"
            >
      ...
      <div className="space-y-2">
        ...
        {displayPrompts.map((prompt) => (
          <button
            key={prompt.id}
            onClick={() => onSelect(prompt.text)}
            className="w-full text-left px-4 py-3 glass-subtle rounded-lg hover:bg-white/5 transition-all flex items-center gap-3 group border border-transparent hover:border-primary-500/20"
          >
            {prompt.icon && (
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <Sparkles className="w-4 h-4 text-primary-400" />
        <p className="text-xs text-neutral-400">Not sure what to build? Try one of these:</p>
      </div>
      <div className="flex flex-wrap gap-2">
        {displayPrompts.map((prompt) => (
          <button
            key={prompt.id}
            onClick={() => onSelect(prompt.text)}
            className="px-3 py-2 rounded-lg glass-subtle hover:bg-primary-500/10 hover:border-primary-500/30 border border-transparent text-xs text-neutral-300 hover:text-primary-400 transition-all flex items-center gap-2"
          >
