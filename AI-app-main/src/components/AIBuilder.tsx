...
import { GeneratedComponent, AppVersion, ChatMessage, LayoutMode, ActiveTab } from '../types/aiBuilderTypes';
...
export default function AIBuilder() {
  ...
  const sendMessage = async () => {
    ...
    try {
        ...
        // 2. ACT MODE
            ...
            const isQuestion = prompt.includes('?') || prompt.toLowerCase().startsWith('how') || prompt.toLowerCase().startsWith('what');
            ...
            if (isQuestion) {
                ...
                // Modify Existing App
                 const response = await fetch('/api/ai-builder/modify', {
                    ...
                    body: JSON.stringify({
                        prompt,
                        currentAppState: state.currentComponent ? JSON.parse(state.currentComponent.code) : null,
                        conversationHistory: optimizedContext
                    })
                });
                const data = await response.json();
  ...
  const handleRunCodeReview = async (forceFullReview = false) => {
    ...
    try {
        ...
        setQualityReport({
            ...
            metrics: { critical: 0, high: 0, medium: 0, low: 0, totalIssues: 0, autoFixableCount: 0 },
            issues: [],
            suggestions: []
        });
  ...
  const handleUndo = () => {
    ...
    dispatch({ 
        ...
        payload: { ...state.currentComponent, code: prev.code, description: prev.description }
    });
  };
  ...
  const handleRedo = () => {
     ...
     dispatch({ 
        type: 'UPDATE_COMPONENT', 
        payload: { ...state.currentComponent, code: next.code, description: next.description }
     });
  };

  // --- RENDER ---

  if (!isClient) return <div className="min-h-screen bg-neutral-950 text-neutral-400 flex items-center justify-center">Loading...</div>;

  return (
    <div className="min-h-screen">
      <BuilderHeader 
        layoutMode={state.layoutMode}
        onLayoutChange={(mode) => dispatch({ type: 'SET_LAYOUT_MODE', payload: mode })}
        themeManager={state.themeManager}
        showVersionHistory={state.showVersionHistory}
        setShowVersionHistory={() => dispatch({ type: 'TOGGLE_VERSION_HISTORY' })}
      ...
      <div className="max-w-7xl mx-auto px-4 py-4">
        ...
        >
          ...
          >
            {guidedBuildMode && implementationPlan ? (
                ...
                <ChatPanel
                    ...
                    onViewComponent={() => dispatch({ type: 'SET_ACTIVE_TAB', payload: 'preview' })}
                    currentMode={currentMode}
                    setCurrentMode={setCurrentMode}
          ...
          `}>
            <div className={`${state.layoutMode === 'stacked' ? 'w-8 h-0.5' : 'w-0.5 h-8'} bg-white/10 group-hover:bg-primary-500/50 transition-colors rounded-full`} />
          </PanelResizeHandle>
          ...
          <Panel defaultSize={70}>
            <PreviewPanel
                ...
                currentComponent={state.currentComponent}
                onSetActiveTab={(tab) => dispatch({ type: 'SET_ACTIVE_TAB', payload: tab })}
                onUndo={handleUndo}
                onRedo={handleRedo}
                ...
                onDownload={() => {
                    if(!state.currentComponent) return;
                    const blob = new Blob([state.currentComponent.code], { type: 'text/plain' });
      ...
      {showQuickStart && (
        <QuickStartSelector
          ...
          onSelect={(t) => { setSelectedQuickStartTemplate(t); setShowQuickStart(false); setShowConceptWizard(true); }}
          onSkip={() => { setSelectedQuickStartTemplate(null); setShowQuickStart(false); setShowConceptWizard(true); }}
        />
      )}
